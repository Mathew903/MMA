...
const Login = () => {
  const [login, setLogin] = useState({ name: "", password: "", rol: 1 })
  ...
  const onLogin = (e) => {
    e.preventDefault()
    try {
      fetch("http://127.0.0.1:8000/login", { method: "POST", headers: {'Content-Type': 'application/json'}, body: JSON.stringify(login)})
        .then(res => res.json())
        .then(data => {
